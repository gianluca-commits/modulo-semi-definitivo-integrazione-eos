...
const GoogleMapsPolygonDrawer = forwardRef<PolygonDrawerRef, PolygonDrawerProps>(({
  ...
  return <Card className="w-full">
        ...
        <CardContent>
          <div ref={mapRef} className="w-full h-[400px] rounded-lg border" style={{
        minHeight: '400px',
        height: '400px',
        width: '100%'
      }} />
          {area && <div className="mt-3 flex justify-center">
              <Badge variant="default">
                Area del campo: {area.toFixed(2)} ha
              </Badge>
            </div>}
          {!isMapLoaded && <div className="absolute inset-0 flex items-center justify-center bg-muted/50 rounded-lg">
