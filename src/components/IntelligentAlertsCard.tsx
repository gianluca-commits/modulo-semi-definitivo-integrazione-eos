...
import { AlertTriangle, Clock, TrendingDown, Euro, Droplets, Sprout, CloudRain, Activity, CheckCircle } from "lucide-react";
...
export function IntelligentAlertsCard({
  alertsBundle
}: IntelligentAlertsCardProps) {
      ...
      <CardContent className="space-y-6">
        ...
        <div className="space-y-3">
          ...
          <p className={`text-sm font-medium ${getRiskScoreColor(alertsBundle.total_risk_score)}`}>
            {alertsBundle.total_risk_score >= 70 ? "Rischio Elevato - Intervento Urgente" : alertsBundle.total_risk_score >= 40 ? "Rischio Moderato - Monitoraggio Attivo" : "Rischio Basso - Situazione Controllata"}
          </p>
        </div>
        ...
        {/* Immediate Actions */}
        {alertsBundle.immediate_actions.length > 0 && <div className="space-y-2">
            <h4 className="font-medium text-sm flex items-center gap-2">
              <Clock className="h-4 w-4 text-warning" />
              Azioni Immediate (48h)
            </h4>
            <div className="space-y-1">
              {alertsBundle.immediate_actions.map((action, index) => <div key={index} className="flex items-center gap-2 p-2 bg-warning/10 rounded text-sm">
        ...
        <div className="space-y-3">
          ...
          {alertsBundle.critical_alerts.map(alert => <Alert key={alert.id} className={`border-l-4 ${alert.severity === "critical" ? "border-l-destructive" : alert.severity === "high" ? "border-l-warning" : "border-l-info"}`}>
              <div className="space-y-3">
                {/* Alert Header */}
                ...
                <div className="flex items-center justify-between text-xs text-muted-foreground bg-muted/30 p-2 rounded">
                  ...
                  <Badge variant="outline" className="text-xs">
                    {alert.triggers.trend_direction === "worsening" ? "⬇️ Peggioramento" : alert.triggers.trend_direction === "improving" ? "⬆️ Miglioramento" : "➡️ Stabile"}
                  </Badge>
                </div>
              </div>
            </Alert>)}
        ...
        {/* Action Button */}
        {alertsBundle.critical_alerts.some(alert => alert.severity === "critical") && <Button className="w-full" variant="destructive">
